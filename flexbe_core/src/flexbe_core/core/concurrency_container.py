...
class ConcurrencyContainer(OperatableStateMachine):
    ...
    def _update_once(self):
        ...
        for state in self._states:
            if state.name in list(self._returned_outcomes.keys()) and self._returned_outcomes[state.name] != self._loopback_name and self._returned_outcomes[state.name] is not None:
                continue
            if (PriorityContainer.active_container is not None
                ...
                continue
            state_sleep_dur = state.sleep_duration
        ...
        if outcome in self.outcomes:
            ...
            raise ValueError("Outcome '%s' of state '%s' with transition target '%s' is neither a registered state nor a registered container outcome." %
